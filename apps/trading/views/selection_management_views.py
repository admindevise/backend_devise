...
# Vistas de conveniencia para mantener compatibilidad

@api_view(['POST'])
@permission_classes([IsAuthenticated])
def create_purchase_selection(request):
    """Vista de conveniencia para crear selecciones de compra"""
    
    # Agregar order_type automáticamente
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def create_sales_selection(request):
    """Vista de conveniencia para crear selecciones de venta"""
    ...
    try:
        ...
        if result.get('warning_type') == 'INSUFFICIENT_BUYERS':
            return Response(result, status=status.HTTP_206_PARTIAL_CONTENT)
