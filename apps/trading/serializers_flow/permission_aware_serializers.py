...
class PermissionAwarePurchaseOrderSerializer(PurchaseOrderSerializer):
    """
    Serializer que valida permisos de admin antes de crear órdenes de compra
    ✅ SIEMPRE valida permisos, incluso para usuarios normales
    """
    
    target_user_id = serializers.IntegerField(
        required=False,
        write_only=True,
        help_text="ID del usuario para quien se crea la orden (solo admins)"
    )
    
    class Meta(PurchaseOrderSerializer.Meta):
        model = PurchaseOrderSerializer.Meta.model
        fields = PurchaseOrderSerializer.Meta.fields + ['target_user_id',]
        extra_kwargs = {
            **getattr(PurchaseOrderSerializer.Meta, 'extra_kwargs', {}),
            'target_user_id': {'write_only': True}
        }
    ...
    def validate(self, attrs):
        """✅ NUEVA LÓGICA: Siempre valida permisos"""
        ...
        if target_user_id:
            # ✅ ESCENARIO 1: Admin creando orden para otro usuario
            ...
            # ✅ CORRECTO: Usuario objetivo como supplier_user
            attrs['supplier_user'] = target_user  # El usuario que realmente compra
            ...
            # ✅ ESCENARIO 2: Usuario creando su propia orden
            target_user = requesting_user
            
            # ✅ VALIDACIÓN BÁSICA OBLIGATORIA: Usuario debe ser inversor válido del fondo
            ...
            attrs['supplier_user'] = target_user  # El mismo usuario que hace la request
        ...
        if self._permission:
            ...
            if self._requires_confirmation:
                ...
                pending_action = TradingPermissionService.request_approval(
                    ...
                    description=f"Crear orden de compra por ${validated_data.get('total_amount')} para {validated_data['supplier_user'].email}"
                )
                ...
                # Ejecutar directamente con permisos
                result = TradingPermissionService.execute_with_permission(
                    permission=self._permission,
                    action_type='CREATE_PURCHASE_ORDER',
                    action_data=validated_data,
                    request=self.context.get('request')
                ...
                if not result['success']:
                    raise serializers.ValidationError(f"Error ejecutando con permisos: {result['error']}")
                
                return result['result']['purchase_order']
        else:
            # ✅ Creación normal (usuario válido ya verificado)
            return super().create(validated_data)


class PermissionAwareSalesOrderSerializer(SalesOrderSerializer):
    """
    ...
    target_user_id = serializers.IntegerField(
        required=False,
        write_only=True,  # ✅ AGREGAR: write_only
        help_text="ID del usuario para quien se crea la orden (solo admins)"
    )
    
    # ✅ AGREGAR: Clase Meta faltante
    class Meta(SalesOrderSerializer.Meta):
        model = SalesOrderSerializer.Meta.model
        fields = SalesOrderSerializer.Meta.fields + ['target_user_id',]
        extra_kwargs = {
            **getattr(SalesOrderSerializer.Meta, 'extra_kwargs', {}),
            'target_user_id': {'write_only': True}
        }
    ...
    def validate(self, attrs):
        ...
        if target_user_id:
            ...
            # ✅ CORRECTO: Usuario objetivo como seller_user
            attrs['seller_user'] = target_user  # El usuario que realmente vende
            self._permission = permission_check.get('permission')
            self._requires_confirmation = permission_check.get('requires_confirmation', False)
        ...
        if self._permission:
            ...
            if self._requires_confirmation:
                ...
                pending_action = TradingPermissionService.request_approval(
                    ...
                    action_data=validated_data,
                    amount=validated_data.get('total_amount'),
                    description=f"Crear orden de venta por ${validated_data.get('total_amount')} para {validated_data['seller_user'].email}"
                )
                ...
                from apps.trading.models.core_models import SalesOrder
                sales_order = SalesOrder.objects.get(id=result['result']['order_id'])
                return sales_order
        else:
            # Creación normal
            return super().create(validated_data)
