...
class FundApplicationSerializer(serializers.ModelSerializer):
    ...
    def validate_fund_id(self, value):
        try:
            ...
            raise serializers.ValidationError(f"Fund with ID {value} does not exist.")
        if fund.status != 'active':
            raise serializers.ValidationError("Cannot apply to a fund that is not active.")
        return value
    ...
    def validate_duplicate_application(self, attrs):
        fund = attrs.get('fund_id')
        applicant = self.context['request'].user
        
        # Verificar si ya existe cualquier aplicación (independiente del status)
        if FundApplication.objects.filter(fund=fund, applicant=applicant).exists():
            raise ValidationError("You already have an application for this fund.")
        return attrs
