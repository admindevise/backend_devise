...
class ListSocioeconomicSerializer(serializers.ModelSerializer):
    origin_of_funds = serializers.StringRelatedField()
    foreign_operations_country = serializers.StringRelatedField()
    country_of_tax_residence = serializers.StringRelatedField()
    
    class Meta:
        model = Socioeconomic
        fields = [
            'id', 'monthly_income', 'other_income', 'value_other_income',
            'monthly_expenses', 'total_assets', 'total_liabilities',
            'origin_of_funds', 'income_explanation', 'manage_public_resources',
            'links_with_pep', 'foreign_currency_operations', 'foreign_operations_country',
            'foreign_operations_value', 'outside_tax_obligation', 'country_of_tax_residence',
